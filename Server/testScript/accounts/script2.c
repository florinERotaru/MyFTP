...
int GetFile(int sd, char* filename)
{
	...
	int newfile=open("cartea1.pdf", O_RDWR|O_TRUNC|O_CREAT, S_IRWXU);
    if(newfile==-1)
    {
...
int HandleSignup(int sd, char* final_username)
{
    ...
    char username[USER_INFO_SIZE]="cont3";
    printf("Set Username:"); fflush(stdout);
    ValidateUsername(username, sd);
    strncpy(final_username, username, USER_INFO_SIZE);
    printf("Set Password:"); fflush(stdout);
    char password[USER_INFO_SIZE]="parola1";
    ValidatePassword(password);
    printf("Confirm Password:"); fflush(stdout);
    char second_password[USER_INFO_SIZE]="parola1";
    confirm_password(password, second_password);    
